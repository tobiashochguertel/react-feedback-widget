/**
 * CanvasOverlay - Drawing overlay for visual feedback
 *
 * Provides a full-screen canvas for users to draw annotations
 * on the current page. Captures screenshots with drawings overlay.
 */
...
interface CanvasOverlayProps {
  isActive: boolean;
  onComplete: (screenshot: string | null, feedback: string) => void;
  onCancel: () => void;
  mode?: ThemeMode;
}
...
const ToolButton = styled.button`
  ...
  &:hover {
    background-color: ${(props) =>
    props.theme.mode === 'dark' ? '#4b5563' : '#e5e7eb'};
  }
`;
...
export const CanvasOverlay: React.FC<CanvasOverlayProps> = ({
  ...
  const [inputPosition, setInputPosition] = useState<Coordinates>({ x: 0, y: 0 });
  ...
  const getCoordinates = useCallback(
    (e: MouseEvent<HTMLCanvasElement> | TouchEvent<HTMLCanvasElement>): Coordinates => {
      const canvas = canvasRef.current;
  ...
  const startDrawing = useCallback(
    (e: MouseEvent<HTMLCanvasElement> | TouchEvent<HTMLCanvasElement>): void => {
      if (!context) return;
  ...
  const handleComplete = useCallback((): void => {
    ...
    // Fallback to center-right
      ...
      y = Math.max(
        20,
        Math.min(centerY - inputHeight / 2, window.innerHeight - inputHeight - 20)
      );
    }
