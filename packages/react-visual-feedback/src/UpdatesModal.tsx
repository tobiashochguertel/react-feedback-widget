/**
 * UpdatesModal - Display changelog and updates
 *
 * Modal component for showing "What's New" updates and changelog
 * with filter capabilities for Fixed issues and New Features.
 */
...
interface UpdatesModalProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdatesSeen?: (updates: UpdateItem[]) => void;
  updates?: UpdateItem[];
  title?: string;
  mode?: ThemeMode;
}
...
const UpdateItemContainer = styled.div<{ $index: number }>`
  padding: 16px 20px;
  border-bottom: 1px solid ${props => props.theme.mode === 'dark'
    ? 'rgba(255, 255, 255, 0.06)'
    : 'rgba(0, 0, 0, 0.05)'};
...
const getTypeStyles = (type: UpdateType, theme: Theme): TypeStyles => {
  switch (type) {
    case 'solved':
      return {
        bg: theme.mode === 'dark' ? 'rgba(16, 185, 129, 0.2)' : '#d1fae5',
        color: '#059669',
      };
    case 'new_feature':
      return {
        bg: theme.mode === 'dark' ? 'rgba(139, 92, 246, 0.2)' : '#ede9fe',
        color: '#7c3aed',
      };
    default:
      return {
        bg: theme.mode === 'dark' ? 'rgba(59, 130, 246, 0.2)' : '#dbeafe',
        color: '#2563eb',
      };
  }
};

const getStatusLabel = (type: UpdateType): string => {
  switch (type) {
    case 'solved':
      return 'Fixed';
    case 'new_feature':
...
const getStatusIcon = (type: UpdateType): React.ReactNode => {
  switch (type) {
    case 'solved':
...
export const UpdatesModal: React.FC<UpdatesModalProps> = ({
  ...
  const [filter, setFilter] = useState<UpdateType | null>('solved');
  ...
  const sortedUpdates = useMemo(() => {
    const typeOrder: Record<UpdateType, number> = { solved: 0, new_feature: 1 };
        ...
        <Content>
          {filteredUpdates.length === 0 ? (
            ...
            filteredUpdates.map((update, index) => {
              ...
              return (
                <UpdateItemContainer key={update.id || index} $index={index}>
                  <ItemHeader>
                  ...
                  <ItemFooter>
                    ...
                    {update.category && <MetaTag>{update.category}</MetaTag>}
                  </ItemFooter>
                </UpdateItemContainer>
              );
            })
